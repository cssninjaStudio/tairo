...
const tasks: Ref<TaskResult<Company>> = data.value.data!
...
<template>
  <div>
    <TairoContentWrapperTabbed :labels="['Active', 'Inactive']">
      ...
      <template #tab-1>
        <div>
          <div v-if="!pending && tasks?.count === 0">
            <BasePlaceholderPage
          ...
          <div v-else class="space-y-4">
            ...
            >
              <template
                v-for="item in tasks?.results"
                :key="item.id"
              >
                <BaseCard
                  v-if="item.task_name !== 'All Task'"
                  rounded="lg"
