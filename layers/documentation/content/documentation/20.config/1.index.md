...
A Nuxt application is configured through the `nuxt.config.ts` file. In this file, we can setup core features of nuxt, enable modules, configure them, extends from other layers, and more.

Each layer has a `nuxt.config.ts` file that is used to configure the layer itself. Layers can also have a `app.config.ts` file that is used to configure the layer's default app configuration, in that case you will also find a `nuxt.schema.ts` file that is used to define the layer's configuration schema.
...
And in the client using same composable, note that you only has access to the `public` variables:
...
With previous example, you can define the `NUXT_SECRET_KEY` and `NUXT_PUBLIC_MAPBOX_TOKEN` value using an environment variable.
...
In addition to nuxt configuration, another file is used to configure the app itself. This file is located at `app.config.ts` and is used to configure the app's layout, theme, and more.
...
App configuration can then be accessed in components using the `useAppConfig` nuxt composable, we use this internally to configure the layout, theme, and more, but you can also use it to configure your own components.
