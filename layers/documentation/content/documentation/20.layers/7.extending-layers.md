...
Sometime you may need to override components used in base layers. Instead of editing them directly, copy them in your `<app>/app/components` folder and edit them there.
...
Instead, you can override the component in your app by creating one in your `<app>/app/components` folder using same name of one you want to override.
...
```vue [<app>/app/components/TairoSidebarNavigation.vue]
...
:::code-group{expandable}
```vue [<app>/app/components/BaseProse.vue]
<script setup lang="ts">
const props = withDefaults(
  defineProps<{
...
Let's say we want to override the `ProseLi` component. Copy the original component in your `<app>/app/components` folder and edit it there.

:::code-group
```vue [<app>/app/components/ProseLi.vue]
<template>
  <BaseListItem>
    <slot />
  </BaseListItem>
