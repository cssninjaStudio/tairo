...
This will find packages in the workspace that has the name `app` in thier `package.json` and install the dependency in it (here it matches `.app/package.json` name).

:::doc-message{color="muted" icon="ph:info"}
We could use `my-backend` from the previous example instead of `app` to install the dependency in the `.backend` pacakge.
:::
