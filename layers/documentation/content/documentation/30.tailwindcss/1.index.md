...
---
Tairo use [Shuriken UI preset](https://github.com/shuriken-ui/tailwind) which extends the default Tailwind CSS configuration without overriding it. This approach allows you to use the default Tailwind CSS configuration and Shuriken UI preset at the same time.


::base-heading{as="h3" weight="light" size="lg" class="not-prose"}
:tairo-toc-anchor{label="Shuriken UI preset" level="3" prefix=" " suffix="¶"}
::

- Components: 
  ::base-paragraph{size="sm"}
  This includes a set of pre-defined utility reusable in components like `nui-focus`, `nui-mask` and `nui-mark`. You can explore them [on github.com](https://github.com/shuriken-ui/tailwind/blob/main/src/plugins/components.ts)
  ::
...
::base-heading{as="h3" weight="light" size="lg" class="not-prose"}
:tairo-toc-anchor{label="Tairo plugins" level="3" prefix=" " suffix="¶"}
::

In addition to tailwind plugin and shuriken ui preset, Tairo add some plugins which you can find in `layers/tairo/tailwind` like the `expose-colors` plugin that expose Tailwind CSS colors in css variables.
