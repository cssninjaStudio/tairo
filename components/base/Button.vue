<script setup lang="ts">
import type { RouteLocationRaw } from 'vue-router'

export type ButtonFlavors = 'solid' | 'outline' | 'pastel'
export type ButtonColors =
  | 'default'
  | 'primary'
  | 'info'
  | 'success'
  | 'warning'
  | 'danger'
export type ButtonShapes = 'straight' | 'rounded' | 'curved' | 'full'

export interface ButtonProps {
  type?: 'button' | 'submit' | 'reset'
  to?: RouteLocationRaw
  disabled?: boolean
  rel?: string
  target?: string

  flavor?: ButtonFlavors
  color?: ButtonColors
  shape?: ButtonShapes
  loading?: boolean
  muted?: boolean
}

const props = withDefaults(defineProps<ButtonProps>(), {
  flavor: 'solid',
  color: 'default',
  shape: 'rounded',
  to: undefined,
  type: undefined,
})

const shapeClass = computed(() => {
  switch (props.shape) {
    case 'straight':
      return ''
    case 'curved':
      return 'rounded-xl'
    case 'full':
      return 'rounded-full'
    case 'rounded':
    default:
      return 'rounded-md'
  }
})
const colorClasses = computed(() => {
  if (props.muted) {
    return [
      'border',
      'text-slate-500',
      'bg-slate-200',
      'border-slate-200',
      'dark:text-white',
      'dark:bg-slate-700',
      'dark:border-slate-700',
      'dark:hover:enabled:bg-slate-600',
      'dark:focus-visible:bg-slate-600',
      'hover:enabled:bg-slate-100',
      'focus-visible:bg-slate-100',
      'active:enabled:bg-slate-200',
      'dark:active:enabled:bg-slate-700',
    ]
  }

  if (props.color === 'default') {
    return [
      'text-slate-700',
      'bg-white',
      'border',
      'border-slate-300',
      'dark:text-white',
      'dark:bg-slate-700',
      'dark:border-slate-600',
      'dark:border-slate-600',
      'dark:hover:enabled:bg-slate-600',
      'dark:focus-visible:bg-slate-600',
      'dark:active:enabled:bg-slate-700',
      'hover:enabled:bg-slate-50',
      'focus-visible:bg-slate-50',
      'active:enabled:bg-white',
    ]
  }

  if (props.flavor === 'solid') {
    switch (props.color) {
      case 'info':
        return [
          'bg-sky-400',
          'dark:bg-sky-500',
          'text-white',
          'hover:enabled:bg-sky-500',
          'dark:hover:enabled:bg-sky-600',
          'hover:enabled:shadow-lg',
          'hover:enabled:shadow-sky-500/50',
          'dark:hover:enabled:shadow-sky-800/20',
          'focus-visible:outline-sky-400/70',
          'focus-within:outline-sky-400/70',
          'focus-visible:bg-sky-500',
          'active:enabled:bg-sky-400',
          'dark:focus-visible:outline-sky-400/70',
          'dark:focus-within:outline-sky-400/70',
          'dark:focus-visible:bg-sky-600',
          'dark:active:enabled:bg-sky-500',
        ]
      case 'success':
        return [
          'bg-teal-400',
          'dark:bg-teal-500',
          'hover:enabled:bg-teal-500',
          'dark:hover:enabled:bg-teal-600',
          'text-white',
          'hover:enabled:shadow-lg',
          'hover:enabled:shadow-teal-500/50',
          'dark:hover:enabled:shadow-teal-800/20',
          'focus-visible:outline-teal-400/70',
          'focus-within:outline-teal-400/70',
          'focus-visible:bg-teal-500',
          'active:enabled:bg-teal-400',
          'dark:focus-visible:outline-teal-400/70',
          'dark:focus-within:outline-teal-400/70',
          'dark:focus-visible:bg-teal-600',
          'dark:active:enabled:bg-teal-500',
        ]
      case 'warning':
        return [
          'bg-amber-400',
          'dark:bg-amber-500',
          'hover:enabled:bg-amber-500',
          'dark:hover:enabled:bg-amber-600',
          'text-white',
          'hover:enabled:shadow-lg',
          'hover:enabled:shadow-amber-500/50',
          'dark:hover:enabled:shadow-amber-800/20',
          'focus-visible:outline-amber-400/70',
          'focus-within:outline-amber-400/70',
          'focus-visible:bg-amber-500',
          'active:enabled:bg-amber-400',
          'dark:focus-visible:outline-amber-400',
          'dark:focus-within:outline-amber-400',
          'dark:focus-visible:bg-amber-600',
          'dark:active:enabled:bg-amber-500',
        ]
      case 'danger':
        return [
          'bg-rose-400',
          'dark:bg-rose-500',
          'hover:enabled:bg-rose-500',
          'dark:hover:enabled:bg-rose-600',
          'text-white',
          'hover:enabled:shadow-lg',
          'hover:enabled:shadow-rose-500/50',
          'dark:hover:enabled:shadow-rose-800/20',
          'focus-visible:outline-rose-400/70',
          'focus-within:outline-rose-400/70',
          'focus-visible:bg-rose-500',
          'active:enabled:bg-rose-400',
          'dark:focus-visible:outline-rose-400',
          'dark:focus-within:outline-rose-400',
          'dark:focus-visible:bg-rose-600',
          'dark:active:enabled:bg-rose-500',
        ]
      case 'primary':
      default:
        return [
          'bg-primary-400',
          'dark:bg-primary-500',
          'hover:enabled:bg-primary-500',
          'dark:hover:enabled:bg-primary-600',
          'text-white',
          'hover:enabled:shadow-lg',
          'hover:enabled:shadow-primary-500/50',
          'dark:hover:enabled:shadow-primary-800/20',
          'focus-visible:outline-primary-400/70',
          'focus-within:outline-primary-400/70',
          'focus-visible:bg-primary-500',
          'active:enabled:bg-primary-400',
          'dark:focus-visible:outline-primary-400',
          'dark:focus-within:outline-primary-400',
          'dark:focus-visible:bg-primary-600',
          'dark:active:enabled:bg-primary-500',
        ]
    }
  } else if (props.flavor === 'pastel') {
    switch (props.color) {
      case 'info':
        return [
          'bg-sky-100',
          'hover:enabled:bg-sky-200',
          'text-sky-500',
          'border-2',
          'border-sky-100',
          'dark:border-sky-500',
          'dark:bg-transparent',
          'dark:text-sky-500',
          'dark:hover:enabled:bg-sky-500/10',
          'focus-visible:outline-sky-400/70',
          'focus-within:outline-sky-400/70',
          'focus-visible:bg-sky-200',
          'active:enabled:bg-sky-100',
          'dark:focus-visible:outline-sky-400/70',
          'dark:focus-within:outline-sky-400/70',
          'dark:focus-visible:bg-sky-500/10',
          'dark:active:enabled:bg-transparent',
        ]
      case 'success':
        return [
          'bg-teal-100',
          'hover:enabled:bg-teal-200',
          'text-teal-500',
          'border-2',
          'border-teal-100',
          'dark:border-teal-500',
          'dark:bg-transparent',
          'dark:text-teal-500',
          'dark:hover:enabled:bg-teal-500/10',
          'focus-visible:outline-teal-400/70',
          'focus-within:outline-teal-400/70',
          'focus-visible:bg-teal-200',
          'active:enabled:bg-teal-100',
          'dark:focus-visible:outline-teal-400/70',
          'dark:focus-within:outline-teal-400/70',
          'dark:focus-visible:bg-teal-500/10',
          'dark:active:enabled:bg-transparent',
        ]
      case 'warning':
        return [
          'bg-amber-100',
          'hover:enabled:bg-amber-200',
          'text-amber-500',
          'border-2',
          'border-amber-100',
          'dark:border-amber-500',
          'dark:bg-transparent',
          'dark:text-amber-500',
          'dark:hover:enabled:bg-amber-500/10',
          'focus-visible:outline-amber-400/70',
          'focus-within:outline-amber-400/70',
          'focus-visible:bg-amber-200',
          'active:enabled:bg-amber-100',
          'dark:focus-visible:outline-amber-400/70',
          'dark:focus-within:outline-amber-400/70',
          'dark:focus-visible:bg-amber-500/10',
          'dark:active:enabled:bg-transparent',
        ]
      case 'danger':
        return [
          'bg-rose-100',
          'hover:enabled:bg-rose-200',
          'text-rose-500',
          'border-2',
          'border-rose-100',
          'dark:border-rose-500',
          'dark:bg-transparent',
          'dark:text-rose-500',
          'dark:hover:enabled:bg-rose-500/10',
          'focus-visible:outline-rose-400/70',
          'focus-within:outline-rose-400/70',
          'focus-visible:bg-rose-200',
          'active:enabled:bg-rose-100',
          'dark:focus-visible:outline-rose-400/70',
          'dark:focus-within:outline-rose-400/70',
          'dark:focus-visible:bg-rose-500/10',
          'dark:active:enabled:bg-transparent',
        ]
      case 'primary':
      default:
        return [
          'bg-primary-100',
          'hover:enabled:bg-primary-200',
          'text-primary-500',
          'border-2',
          'border-primary-100',
          'dark:border-primary-500',
          'dark:bg-transparent',
          'dark:text-primary-500',
          'dark:hover:enabled:bg-primary-500/10',
          'focus-visible:outline-primary-400/70',
          'focus-within:outline-primary-400/70',
          'focus-visible:bg-primary-200',
          'active:enabled:bg-primary-100',
          'dark:focus-visible:outline-primary-400/70',
          'dark:focus-within:outline-primary-400/70',
          'dark:focus-visible:bg-primary-500/10',
          'dark:active:enabled:bg-transparent',
        ]
    }
  } else if (props.flavor === 'outline') {
    switch (props.color) {
      case 'info':
        return [
          'text-sky-500',
          'hover:enabled:text-white',
          'border-2',
          'border-sky-500',
          'hover:enabled:bg-sky-500',
          'focus-within:outline-sky-400/70',
          'focus-visible:outline-sky-400/70',
          'focus-visible:bg-sky-500',
          'focus-visible:text-white',
          'active:enabled:text-sky-500',
          'active:enabled:bg-transparent',
        ]
      case 'success':
        return [
          'text-teal-500',
          'hover:enabled:text-white',
          'border-2',
          'border-teal-500',
          'hover:enabled:bg-teal-500',
          'focus-within:outline-teal-400/70',
          'focus-visible:outline-teal-400/70',
          'focus-visible:bg-teal-500',
          'focus-visible:text-white',
          'active:enabled:text-teal-500',
          'active:enabled:bg-transparent',
        ]
      case 'warning':
        return [
          'text-amber-500',
          'hover:enabled:text-white',
          'border-2',
          'border-amber-500',
          'hover:enabled:bg-amber-500',
          'focus-within:outline-amber-400/70',
          'focus-visible:outline-amber-400/70',
          'focus-visible:bg-amber-500',
          'focus-visible:text-white',
          'active:enabled:text-amber-500',
          'active:enabled:bg-transparent',
        ]
      case 'danger':
        return [
          'text-rose-500',
          'hover:enabled:text-white',
          'border-2',
          'border-rose-500',
          'hover:enabled:bg-rose-500',
          'focus-within:outline-rose-400/70',
          'focus-visible:outline-rose-400/70',
          'focus-visible:bg-rose-500',
          'focus-visible:text-white',
          'active:enabled:text-rose-500',
          'active:enabled:bg-transparent',
        ]
      case 'primary':
      default:
        return [
          'text-primary-500',
          'hover:enabled:text-white',
          'border-2',
          'border-primary-500',
          'hover:enabled:bg-primary-500',
          'focus-within:outline-primary-400/70',
          'focus-visible:outline-primary-400/70',
          'focus-visible:bg-primary-500',
          'focus-visible:text-white',
          'active:enabled:text-primary-500',
          'active:enabled:bg-transparent',
        ]
    }
  }

  return []
})
const buttonClasses = computed(() => [
  'relative',
  'font-text font-normal text-sm no-underline',
  'inline-flex justify-center items-center',
  'leading-5',
  'h-10',
  'px-5 py-2 space-x-1',
  'tairo-focus',
  'transition-colors transition-shadow',
  'duration-300',
  props.loading && '!text-transparent',
  props.disabled &&
    'disabled:opacity-60 disabled:cursor-not-allowed hover:shadow-none',
  shapeClass.value,
  ...colorClasses.value,
])

const { attributes, is } = useButton(props)
</script>

<template>
  <component :is="is" v-bind="attributes" :class="buttonClasses">
    <slot v-if="!props.loading"></slot>
    <BasePlaceload v-else class="h-4 w-12 rounded" />
  </component>
</template>
