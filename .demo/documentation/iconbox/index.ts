export const roundedSizes = `
<template>
  <BaseIconBox
    size="xs"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-3 h-3" />
  </BaseIconBox>
  <BaseIconBox
    size="sm"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-4 h-4" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="lg"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-6 h-6" />
  </BaseIconBox>
  <BaseIconBox
    size="xl"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-7 h-7" />
  </BaseIconBox>
  <BaseIconBox
    size="2xl"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-8 h-8" />
  </BaseIconBox>
</template>
`

export const circleSizes = `
<template>
  <BaseIconBox
    size="xs"
    shape="circle"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-3 h-3" />
  </BaseIconBox>
  <BaseIconBox
    size="sm"
    shape="circle"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-4 h-4" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="lg"
    shape="circle"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-6 h-6" />
  </BaseIconBox>
  <BaseIconBox
    size="xl"
    shape="circle"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-7 h-7" />
  </BaseIconBox>
  <BaseIconBox
    size="2xl"
    shape="circle"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:nut-duotone" class="w-8 h-8" />
  </BaseIconBox>
</template>
`

export const solid = `
<template>
  <BaseIconBox
    size="md"
    shape="circle"
    class="bg-white border border-muted-200 text-gray-500 dark:bg-muted-700 dark:border-muted-600"
    >
    <Icon name="ph:nut-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="bg-gray-200 text-gray-500 dark:bg-gray-800"
    >
    <Icon name="ph:cat-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="bg-primary-500 text-white"
    >
    <Icon name="ph:ghost-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="bg-lime-500 text-white"
    >
    <Icon name="ph:club-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="bg-green-500 text-white"
    >
    <Icon name="ph:leaf-duotone" class="w-5 h-5" />
  </BaseIconBox>
</template>
`

export const pastel = `
<template>
  <BaseIconBox
    size="md"
    class="bg-primary-100 text-primary-500 dark:bg-primary-500/20 dark:text-primary-400 dark:border-2 dark:border-primary-500"
    >
    <Icon name="ph:nut-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    class="bg-lime-100 text-lime-500 dark:bg-lime-500/20 dark:text-lime-400 dark:border-2 dark:border-lime-500"
    >
    <Icon name="ph:cookie-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    class="bg-green-100 text-green-500 dark:bg-green-500/20 dark:text-green-400 dark:border-2 dark:border-green-500"
    >
    <Icon name="ph:armchair-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    class="bg-emerald-100 text-emerald-500 dark:bg-emerald-500/20 dark:text-emerald-400 dark:border-2 dark:border-emerald-500"
    >
    <Icon name="ph:confetti-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    class="bg-teal-100 text-teal-500 dark:bg-teal-500/20 dark:text-teal-400 dark:border-2 dark:border-teal-500"
    >
    <Icon name="ph:flask-duotone" class="w-5 h-5" />
  </BaseIconBox>
</template>
`

export const outline = `
<template>
  <BaseIconBox
    size="md"
    shape="circle"
    class="text-gray-400 dark:text-gray-500"
    bordered
    >
    <Icon name="ph:popcorn-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="text-primary-500"
    bordered
    >
    <Icon name="ph:presentation-chart-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="text-lime-500"
    bordered
    >
    <Icon name="ph:paint-roller-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="text-green-500"
    bordered
    >
    <Icon name="ph:rocket-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="text-emerald-500"
    bordered
    >
    <Icon name="ph:package-duotone" class="w-5 h-5" />
  </BaseIconBox>
</template>
`

export const elevation = `
<template>
  <BaseIconBox
    size="md"
    shape="circle"
    class="bg-white border border-muted-200 text-gray-500 dark:bg-muted-700 dark:border-muted-600 shadow-lg"
    >
    <Icon name="ph:nut-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="bg-primary-500 text-white shadow-lg shadow-primary-500/50"
    >
    <Icon name="ph:ghost-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="bg-lime-500 text-white shadow-lg shadow-lime-500/50"
    >
    <Icon name="ph:club-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="bg-green-500 text-white shadow-lg shadow-green-500/50"
    >
    <Icon name="ph:leaf-duotone" class="w-5 h-5" />
  </BaseIconBox>
  <BaseIconBox
    size="md"
    shape="circle"
    class="bg-emerald-500 text-white shadow-lg shadow-emerald-500/50"
    >
    <Icon name="ph:mountains-duotone" class="w-5 h-5" />
  </BaseIconBox>
</template>
`
