  ...
  <div class="relative flex gap-[1px] p-1 h-full">
    ...
    >
      <Icon
        ...
        @pointerdown="
          (e: MouseEvent | TouchEvent) => emits('positiondrag', e)
        "
      />
    </div>
    <div class="[&>:not(:first-of-type):before]:mx-1 pe-1 w-full">
      ...
      <div class="pt-2 mt-1 border-t border-muted-200 dark:border-muted-700 flex justify-between items-center">
        <div class="flex flex-wrap gap-2">
          <span v-if="event.customData.features?.record" class="text-muted-400" data-nui-tooltip="Recorded">
            <Icon name="ph:monitor-play-duotone" class="w-4 h-4" />
          </span>
          <span v-if="event.customData.features?.drive" class="text-muted-400" data-nui-tooltip="Document joined">
            <Icon name="ph:note-duotone" class="w-4 h-4" />
          </span>
          <span v-if="event.customData.features?.external" class="text-muted-400" data-nui-tooltip="External Users">
            <Icon name="ph:lock-open-duotone" class="w-4 h-4" />
          </span>
          <span v-if="event.customData.features?.conversation" class="text-muted-400" data-nui-tooltip="Has comments"> 
            <Icon name="ph:chats-circle-duotone" class="w-4 h-4" />
          </span>
        </div>
        <div>
          <div class="w-2 h-2 rounded-full" :class="[categoryTheme[event.customData.category].color]" :data-nui-tooltip="categoryTheme[event.customData.category].name" data-nui-tooltip-position="start"></div>
        </div>
      </div>
    </div>
  </div>
